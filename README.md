...
```math
Полное\,время\,выполнения\,hash\_table\_find\,уменьшилось\,в\,\frac{266\,760\,300}{240\,096\,500} \approx 1.11 \,раза
```
<br>
...
Как видно из кода в реализации **O2**, как было отмечено в конце предыдущего раздела, компилятор реализует вызов ***list_find*** через `jump`, а не через `call`, поэтому сохранённые регистры, в том числе `rbx` восстанавливаются до ***list_find***.
