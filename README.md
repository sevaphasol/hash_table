# Поиск и оптимизация хеш-таблицы
...
<div class="toc">
  ...
  <div class="toc-item">
    ...
    <div class="toc-subitem">&emsp;&emsp;4.7 <a href="#47-смена-флага-оптимизации">Смена флага оптимизации</a></div>
  </div>
...
1. **Реализовать базовую версию хеш-таблицы** <br>
   Разработать хеш-таблицу с использованием метода цепочек на языке C. Реализация должна включать основные операции:
...
4. **Оптимизировать узкие места в программе** <br>
   Применить методы оптимизации для устранения выявленных узких мест. В рамках работы рассмотреть следующие подходы:
   - Использование интринсиков для обсчёта хеша.
   - Векторизация кода для ускорения вычислений.
...
```math
Полное\,время\,выполнения\,crc32\_hash\,уменьшилось\,в\,\frac{256\,282\,459}{125\,365\,341} \approx 2.04 \,раза
```
<br>
...
```math
Полное\,время\,выполнения\,hash\_table\_find\,уменьшилось\,в\,\frac{742\,551\,500}{240\,096\,500} \approx 3.09 \,раза.
```
<br>
...
Использование оптимизаций с помощью ассемблера позволило дополнительно ускорить программу в $\approx1.29$ раза, а применение интринсиков для расчёта хеша дало улучшение производительности в $\approx1.18$ раза.
