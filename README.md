...
- Исследовать методы оптимизации с помощью векторизации и интринсиков.
...
***Хеш-таблица*** — это структура данных, которая позволяет хранить пары "ключ-значение" и поддерживает основные операции: добавление, удаление и поиск элементов по ключу. В рамках данной работы реализованы только операции добавления и поиска.

<div align="center">
  <br>
  <strong>Принцип работы хеш-таблицы методом цепочек</strong><br><br>
  <img src="images/hash_table_theory.png" width="600" alt="hash_table_theory">
  <br>
  <br>
</div>
...
Процент ускорения достаточно большой, так что можно оптимизировать дальше.
...
Из написанного выше становится ясно, что ***__strcmp_avx2*** тратит много ресурсов на различные проверки, т.к. она обязана работать в общем случае. Однако можно ускорить её работу, ограничив общность. Это даёт идею новой оптимизации — векторизация.
...
Процент ускорения достаточно маленький, так что на этом этапе можно прекратить оптимизацию и переходить к итогам.
...
<ul style="list-style-type: none; padding-left: 0;">
  ...
  <li><a id="bib6"></a>[7] <a href="https://habr.com/ru/companies/timeweb/articles/828068/">Отсюда была взята картинка с хеш-таблицей</a></li>
</ul>
