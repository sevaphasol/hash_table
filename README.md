...
Как видно из графиков, минимальное значение дисперсии у распределения хеш-функции crc32. Поэтому она и будет использоваться. *Load factor* в данных экспериментах равен ≈ $7.7$. Таблица специально создаётся слишком маленькой, чтобы искусственно повышать *load factor*.
